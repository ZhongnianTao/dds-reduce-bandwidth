...
class Region:
    ...
    def enlarge_absolute(self, delta):
        if delta >= 0:
            x_min = max(self.x - delta, 0.0)
            y_min = max(self.y - delta, 0.0)
            x_max = min(self.x + self.w + delta, 1.0)
            y_max = min(self.y + self.h + delta, 1.0)
            self.x = x_min
            self.y = y_min
            self.w = x_max - x_min
            self.h = y_max - y_min
            ...
            delta = - delta
            x_min = self.x
            x_max = min(self.x + self.w, 1.0)
            y_min = self.y
            y_max = min(self.y + self.h, 1.0)
            x_mid = (x_min + x_max) * 0.5
            y_mid = (y_min + y_max) * 0.5
            self.x = max(x_mid - 0.5 * delta * self.w, 0.0)
            self.y = max(y_mid - 0.5 * delta * self.h, 0.0)
            self.w = self.w * delta
            self.h = self.h * delta
...
def compute_regions_size(results, vid_name, images_direc, resolution, qp,
                         enforce_iframes, estimate_banwidth=True, iter_ratio=None):
    if estimate_banwidth:
        # If not simulation, compress and encode images
        # and get size
        # if iter_ratio is not None:
        #     vid_name = f"{vid_name}-cropped-{iter_ratio}"
        # else:
...
def cleanup(vid_name, debug_mode=False, start_id=None, end_id=None, iter_ratio=None):
    if not os.path.isdir(vid_name + "-cropped"):
        return

    if not debug_mode:
        # if iter_ratio is not None:
        #     shutil.rmtree(f"{vid_name}-base-phase-cropped-{iter_ratio}")
        #     shutil.rmtree(f"{vid_name}-cropped-{iter_ratio}")
        # else:
        shutil.rmtree(f"{vid_name}-base-phase-cropped")
        shutil.rmtree(f"{vid_name}-cropped")
    else:
...
def evaluate(max_fid, map_dd, map_gt, gt_confid_thresh, mpeg_confid_thresh,
    ...
    for fid in range(max_fid+1):
        ...
        for b_gt in bboxes_gt:
            ...
            if not found:
                fn += 1
                # fn_region_list.append((fid, b_gt.x, b_gt.y, b_gt.w, b_gt.h, b_gt.conf, b_gt.label, b_gt.origin))
                print(fid, b_gt)
            else:
                count += 1
        tp_list.append(tp)
